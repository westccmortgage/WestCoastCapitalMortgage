...
(function () {
  ...
  var STEPS = [
    {
      target: "zip",
      title: "Start with the property county",
      body: "Choose the Florida county where the property is located. K West Mortgage serves Key West & Monroe County and all of Florida — Monroe is selected by default. Each county has its own loan size limits, so this sets the baseline for everything that follows.",
    ...
    },
    {
      ...
      body: "Every Florida county has a cut-off loan amount — and in higher-cost areas like Monroe County (Key West) that line sits higher than most. Stay under it and your loan is a regular “conforming” loan; go over it and it becomes a bigger “jumbo” loan. This bar shows where your loan sits versus that line.",
    ...
    },
    {
      ...
      body: "Putting it all together, your scenario fits either conforming (under your Florida county's limit) or jumbo (above it). Neither is automatically better — the goal is the most efficient structure for your situation.",
    ...
    },
    {
      ...
      body: "That's the full walkthrough. Mortgage financing is rarely about a single number — the strongest strategy comes from seeing how the factors work together. When you're ready, a licensed K West Mortgage professional can review your Florida scenario in detail. Everything here is educational — no credit pull, nothing sent anywhere.",
  ...
  var prefMuted = false; // sound ON by default; remembers the user's choice
  ...
  function buildOverlay() {
    ...
    root.innerHTML =
      ...
      '<div class="cmtut__card" data-tut-card>' +
        ...
        '<div class="cmtut__media" data-tut-media>' +
          ...
          '<button type="button" class="cmtut__playbig" data-tut-bigplay aria-label="Play video" hidden>' +
    ...
    els.bigplay = root.querySelector("[data-tut-bigplay]");
    ...
    els.mute.addEventListener("click", function (e) {
      ...
      prefMuted = els.video.muted;        // remember across steps
    ...
    // Big centered Play overlay — shown whenever the clip is paused (e.g. when
    // a mobile browser blocks autoplay). A tap is a user gesture, so play WITH
    // sound and remember that choice for the rest of the tour.
    els.bigplay.addEventListener("click", function (e) {
      e.preventDefault();
      els.video.muted = false; prefMuted = false;
      els.mute.innerHTML = "&#128266;"; els.mute.setAttribute("aria-label", "Mute");
      try { var p = els.video.play(); if (p && p.catch) p.catch(function () {}); } catch (e2) {}
    });
    // Toggle the big Play overlay with the video's play/pause state.
    els.video.addEventListener("play", syncBigPlay);
    els.video.addEventListener("pause", syncBigPlay);
    // Continuous presentation: roll into the next episode when a clip ends.
    els.video.addEventListener("ended", function () {
      if (state.idx < state.order.length - 1) advance();
      else syncBigPlay();
    });

    // If the clip is wrong/missing, fall back to the avatar image silently.
    els.video.addEventListener("error", function () { showLayer("img"); els.mute.hidden = true; if (els.bigplay) els.bigplay.hidden = true; });
  ...
  function loadStepMedia(step) {
    ...
    if (step.videoSrc && v) {
      ...
      // leave the unmute button + big Play overlay so they can enable it.
      v.muted = prefMuted;
      if (els.mute) {
        els.mute.hidden = false;
        els.mute.innerHTML = v.muted ? "&#128263;" : "&#128266;";
        els.mute.setAttribute("aria-label", v.muted ? "Unmute" : "Mute");
      }
      showLayer("video");
      var p = v.play();
      if (p && typeof p.catch === "function") p.catch(function () {
        if (!prefMuted) {
          v.muted = true;
          if (els.mute) { els.mute.innerHTML = "&#128263;"; els.mute.setAttribute("aria-label", "Unmute"); }
          try { var p2 = v.play(); if (p2 && p2.catch) p2.catch(function () { syncBigPlay(); }); } catch (e2) { syncBigPlay(); }
        } else { syncBigPlay(); }
      });
      window.setTimeout(syncBigPlay, 60);
    } else {
