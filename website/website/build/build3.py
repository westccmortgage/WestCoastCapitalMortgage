...
from chrome import (head, header, footer, chat, page_hero, cta_band,
                    PHONE, PHONE_TEL, NMLS, PROGRAMS, IMG, FOUNDER_IMG)
...
def loan_page(slug, name, emoji, hero_sub, intro, sections, faqs, highlights):
    ...
    h += header("programs")
    h += page_hero(f'<a href="conventional.html">Loan Programs</a> / {name}', name, hero_sub, bg=IMG['keys'])
    # highlight strip
...
ab += header("about")
ab += page_hero("About","Honest guidance. Clear answers. Strong results.","Meet the founder behind West Coast Capital Mortgage Inc.", bg=IMG['interior'])
ab += f"""
  ...
  <div class="split">
    <div class="fade">
      ...
      <div style="margin-top:18px">
        <b style="font-family:var(--serif);font-size:1.3rem;color:var(--ink)">Anatoliy Kanevsky</b>
        <span class="muted" style="display:block">Founder &amp; CEO · California Broker since July 16, 2009 · {NMLS}</span>
      </div>
    </div>
    <div class="fade">
...
rv += header("reviews")
rv += page_hero("Reviews","What our clients say","Real experiences from families and investors we've helped finance their futures.", bg=IMG['family'])
rv += f"""
...
bl += header("blog")
bl += page_hero("Blog","The Mortgage Blog","Plain-English guides to help you make smart, confident decisions about your home loan.", bg=IMG['la'])
bl += f"""
<section class="section"><div class="container">
