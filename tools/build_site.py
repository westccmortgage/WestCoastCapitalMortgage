...
# ---- Supabase visitor tracking (shared with Suncoast; same project, tagged by SITE) ----
SITE = "westccmortgage.com"          # overridden by build_suncoast.py
ACCENT = "#0073e6"                    # consent-banner accent (Suncoast overrides to gold)
SUPABASE_URL = "https://npxibfmhrlgqiubfkouu.supabase.co"
SUPABASE_ANON_KEY = "sb_publishable_zzYFKg8Cdd_rKafqkKCCpw_nmnFijYG"
TRACKING_SCRIPTS = '\n<script src="assets/cm-config.js"></script>\n<script src="assets/cm-tracking.js" defer></script>'

def cm_config_js(site, accent):
    return (f'window.CM_CONFIG={{SUPABASE_URL:"{SUPABASE_URL}",'
            f'SUPABASE_ANON_KEY:"{SUPABASE_ANON_KEY}",SITE:"{site}",ACCENT:"{accent}"}};\n')

# Self-contained, dependency-free tracker (REST inserts; no SDK, no modules).
# Privacy: anonymous first-party visitor_id; events only after "Accept & Continue".
CM_TRACKING_JS = r'''/* California Mortgage tracking (shared, Supabase-only via REST). Privacy-conscious. */
(function(){
  "use strict";
...
<script src="script.js"></script>{TRACKING_SCRIPTS}
</body>
</html>"""
...
<section class="bg-light"><div class="wrap">
  <div class="rate-snap">
    ...
    <div class="rate-board" data-src="assets/rates.json" data-limit="5"><p class="muted">Loading current rates&hellip;</p></div>
  </div>
</div></section>
...
<section><div class="wrap">
  ...
  <div class="rate-board" data-src="assets/rates.json"><p class="muted">Loading current rates&hellip;</p></div>
...
"""
PAGES["rates.html"] = dict(title="Today's Mortgage Rates", desc="Today's sample mortgage rates from West Coast Capital Mortgage — illustrative estimates for well-qualified borrowers across conventional, FHA, VA, USDA, jumbo, Non-QM, and DSCR. Not a quote or commitment to lend.", nav="rates", body=_rates())
...
def main():
    ...
    with open(os.path.join(OUT, "assets", "cm-config.js"), "w", encoding="utf-8") as f: f.write(cm_config_js(SITE, ACCENT))
    with open(os.path.join(OUT, "assets", "cm-tracking.js"), "w", encoding="utf-8") as f: f.write(CM_TRACKING_JS)
